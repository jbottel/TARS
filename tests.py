...
class RemoteControlTestCase(unittest.TestCase):
    ...
    def test_playpause(self):
        """Check that the play/pause functionality properly
        pauses and playes the Kodi player when requested.
        """
        # Play the first movie available in the database (will fail if movie
        # database is empty)
        self.xbmc.Player.Open({'item': {'movieid': 1}})

        # Remote Control: Pause
        self.c.get('/remote/playpause')
        # Check 'speed' attribute to check
        player_properties = self.xbmc.Player.GetProperties(
            {"playerid": 1, "properties": ["speed"]})["result"]
    ...
    def test_stop(self):
        """Check that the stop functionality properly stops
        the Kodi player when requested.
        """
        # Play the first movie available in the database (will fail if movie
        # database is empty)
        self.xbmc.Player.Open({'item': {'movieid': 1}})
    ...
    def test_fast_forward(self):
        """Check that the fast forward functionality properly
        fast-forwards the Kodi player when requested.
        """
        # Play the first movie available in the database (will fail if movie
        # database is empty)
        self.xbmc.Player.Open({'item': {'movieid': 1}})
...
class ApplicationFunctionsTestCase(unittest.TestCase):
    ...
    def test_set_volume(self):
        """Check that set_volume() functions changes the
        Kodi system volume when requested.
        """

        current_volume = self.xbmc.Application.GetProperties(
            {"properties": ["volume", "muted"]})["result"]["volume"]
        ...
        self.c.get('/set/volume/5')
        volume_properties = self.xbmc.Application.GetProperties(
            {"properties": ["volume", "muted"]})["result"]
        self.assertEqual(volume_properties["volume"], 5)
...
class TARSSupportFunctionTestCase(unittest.TestCase):
    ...
    def test_format_runtime(self):
        """Check that format_runtime functions properly
        for both runtime styles.
        """
